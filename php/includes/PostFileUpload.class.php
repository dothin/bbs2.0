 ...
 * @Last Modified time: 2016-05-15 16:17:16
 */
//上传文件类
class PostFileUpload{
    ...
    public function __construct($file,$maxSize){
        ...
        $this->postPath=$this->path.'post/';
        $this->linkToday=date('Ymd').'/';
        $this->today=$this->postPath.$this->linkToday;
    ...
    private function setNewName(){
        ...
        $this->linkPath=UPDIR.'post/'.$this->linkToday.$newName;
    ...
    private function checkPath(){
        ...
        if (!is_dir($this->postPath) || !is_writeable($this->postPath)) {
            if (!mkdir($this->postPath, 0777)) {
                chmod($this->path, 0777);
                Tool::alertBack('上传POST创建失败');
            }
        }
